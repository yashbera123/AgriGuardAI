...
# ---------------------------------------------------------------------------
from services.ai_advisor import SAFETY_DISCLAIMER
from services.ai_advisor import chat_with_advisor as _fallback_chat  # noqa: E402
from services.ai_advisor import generate_advisory as _fallback_advisory
...
def _parse_advisory_text(raw_text: str) -> dict:
    ...
    markers = [
        ...
        (
            "sustainability",
            ["sustainable farming", "sustainability", "eco-friendly", "3."],
        ),
        ("treatment_strategy", ["treatment strategy", "treatment plan", "4."]),
        (
            "long_term_advice",
            ["long-term", "long term", "crop protection advice", "5."],
        ),
    ]
...
def generate_advisory(
    ...
    if source == "local":
        result = (
            ...
            else _fallback_advisory(disease, cause, symptoms, medication)
        )
...
def chat_with_advisor(
    ...
    if detected_disease:
        disease_display = detected_disease.replace("Tomato_", "Tomato ").replace(
            "_", " "
        )
        disease_context_block = (
