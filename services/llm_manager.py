...
_raw_gemini_key = GEMINI_API_KEY.strip()

logger.info("=" * 55)
logger.info("LLM MANAGER — PROVIDER INIT")
logger.info("=" * 55)
...
if _valid_key(_raw_gemini_key):
    try:
        ...
        if _test_text:
            ...
            logger.info(f"[OK]  Gemini:  {PROVIDER_GEMINI} — connected")
        else:
            _gemini_init_error = "Empty test response from Gemini"
            logger.warning("Gemini: empty test response — will skip")

    except ImportError:
        _gemini_init_error = "google-genai not installed"
        logger.error(f"Gemini: {_gemini_init_error}")
    except Exception as exc:  # noqa: BLE001
        from utils.exceptions import AdvisorError

        _gemini_init_error = str(exc)
        logger.error(f"Gemini: {exc}")
        # In a strict environment, we could raise AdvisorError(f"Gemini init failed: {exc}")
else:
    _gemini_init_error = "GEMINI_API_KEY missing or placeholder"
    logger.info(f"[SKIP] Gemini: {_gemini_init_error}")
...
_raw_groq_key = GROQ_API_KEY.strip()
...
if _valid_key(_raw_groq_key):
    try:
        ...
        if _groq_text:
            ...
            logger.info(f"[OK]  Groq:    {PROVIDER_GROQ} — connected")
        else:
            _groq_init_error = "Empty test response from Groq"
            logger.warning("Groq:   empty test response — will skip")

    except ImportError:
        _groq_init_error = "groq package not installed (run: pip install groq)"
        logger.error(f"Groq: {_groq_init_error}")
    except Exception as exc:  # noqa: BLE001
        from utils.exceptions import AdvisorError

        _groq_init_error = str(exc)
        logger.error(f"Groq: {exc}")
else:
    _groq_init_error = "GROQ_API_KEY missing or placeholder"
    logger.info(f"[SKIP] Groq: {_groq_init_error}")

logger.info("Local KB always available as final fallback")
logger.info("=" * 55)
