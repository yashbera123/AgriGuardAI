...
from services.prediction_logger import append_prediction, load_history
...
def get_prediction_context(result, crop_config):
    ...
    return {
        ...
        "sustainability": disease_data.get(
            "sustainability", "Information unavailable."
        ),
...
def log_prediction_once(history_df, image_fingerprint, context):
    ...
    log_key = (
        f"{image_fingerprint}:{context['predicted_class']}:{context['confidence']:.2f}"
    )
...
def render_sidebar(crop_config):
    ...
    with st.sidebar:
        ...
        if current == PROVIDER_GEMINI:
            st.markdown(
                '<div class="sb-pill g"> Active: Gemini 2.5 Flash</div>',
                unsafe_allow_html=True,
            )
        elif current == PROVIDER_GROQ:
            st.markdown(
                '<div class="sb-pill q"> Active: Groq Llama 3.3 70B</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                '<div class="sb-pill l"> Active: Local Knowledge Base</div>',
                unsafe_allow_html=True,
            )
...
def render_advisor_section(
    advisor_data: dict, advisor_source: str, advisor_provider: str
):
...
def render_prediction_results(
    ...
    if vsource == "local":
        vsource_html = (
            '<span class="vsource local"> Local Validation (Gemini skipped)</span>'
        )
    elif vsource == "gemini":
...
def render_analytics_section(history_df):
    ...
    v1.metric(
        "Gemini Validation Calls", gcalls, help="Images verified by Gemini Vision API"
    )
    v2.metric(
        "Local Validation Passes",
        lpasses,
