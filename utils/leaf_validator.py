...
def validate_tomato_leaf(image: Image.Image) -> tuple[bool, str]:
    ...
    if gemini_client is None:
        logger.info("leaf_validator: Gemini unavailable — skipping visual validation.")
        return True, "skipped"
...
def _parse_label(raw_text: str) -> str:
    ...
    for label in _KNOWN_LABELS:
        if label in raw_text:
            return label
    logger.warning(
        "Unrecognised Gemini label in response: %r — treating as UNCLEAR", raw_text
    )
    return "UNCLEAR"
...
def _get_gemini_client():
    ...
    """
    try:
        import services.llm_manager as _lm
...
# ---------------------------------------------------------------------------
# NEW: Surface quality metrics for UI display (does NOT replace validate_image_quality)
# ---------------------------------------------------------------------------
def get_image_metrics(image: Image.Image) -> dict:
    """
    Return computed image quality metrics as a dict for UI display.

    Keys:
        width, height, brightness, blur_score, leaf_coverage_pct
    These values are computed the same way as in validate_image_quality()
    but returned for rendering, not for pass/fail gating.
    """
    w, h = image.size

    grayscale = image.convert("L")
    pixels = list(grayscale.getdata())
    mean_val = round(sum(pixels) / len(pixels), 1)

    edges = grayscale.filter(ImageFilter.FIND_EDGES)
    edge_data = list(edges.getdata())
    edge_mean = sum(edge_data) / len(edge_data) if edge_data else 0
    edge_sq = (
        sum((v - edge_mean) ** 2 for v in edge_data) / len(edge_data)
        if edge_data
        else 0
    )
    edge_std = round(edge_sq**0.5, 1)

    # Estimate leaf coverage via green-channel dominance in RGB
    try:
        rgb = image.convert("RGB")
        rgb_pixels = list(rgb.getdata())
        green_dominant = sum(1 for r, g, b in rgb_pixels if g > r and g > b and g > 60)
        leaf_coverage = round(green_dominant / len(rgb_pixels) * 100, 1)
    except Exception:  # noqa: BLE001
        leaf_coverage = 0.0
